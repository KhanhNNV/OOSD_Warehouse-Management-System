...
export default function PutAwayPage() {
  ...
  // ✅ NEW: State cho checkbox "Mark Location Full"
  ...
  // ✅ UPDATE: Submit Handler - truyền isLocationFull
    ...
    <div className="space-y-4 animate-in slide-in-from-left">
      <div className="flex items-center justify-between px-1">
        <span className="text-xs font-bold text-slate-500 uppercase">
          Hàng đang giữ ({transitList.length})
        </span>
        <Button
          variant="ghost"
          size="icon"
                ...
                <div className="bg-slate-50 p-2 rounded border border-dashed border-slate-200 flex flex-wrap gap-2 items-center">
                  ...
                  {suggestedShelves.length > 0 ? (
                    suggestedShelves.map((shelf) => (
                      <span
                        key={shelf}
                        className="text-[10px] font-bold px-1.5 py-0.5 bg-green-100 text-green-700 rounded border border-green-200"
                      >
                        {shelf}
                      </span>
                    ))
    ...
    <div className="space-y-4 animate-in slide-in-from-right">
      <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
        ...
        <div className="text-xs text-blue-600 mt-1">
          Barcode: {session.selectedItem?.barcode}
        </div>
      </div>
  ...
  // ✅ UPDATE: renderInputDetails - Thêm Checkbox
  const renderInputDetails = () => (
    <div className="space-y-4 animate-in slide-in-from-right">
      ...
      <div className="flex items-center gap-2 text-sm bg-green-50 p-3 rounded border border-green-200 text-green-800">
        ...
        <div>
          <div>
            Vị trí: <span className="font-bold">{scannedShelf}</span>
          </div>
      ...
      <div className="space-y-3">
        ...
        {/* ✅ NEW: Checkbox "Mark Location Full" */}
        <div className="flex items-start space-x-3 bg-amber-50 p-3 rounded-lg border border-amber-200">
          ...
          <div className="flex-1">
            ...
            >
              Đánh dấu kệ này đã đầy
            </label>
            <p className="text-xs text-amber-700 mt-1">
              Kệ sẽ không được gợi ý cho lần cất hàng tiếp theo
            </p>
          </div>
        </div>
        ...
        {/* Hạn SD */}
        <div>
          <Label>Hạn sử dụng (optional)</Label>
          <Input
            type="date"
            value={formData.exp}
            onChange={(e) =>
      ...
      <div className="flex gap-2 pt-4">
        <Button
          ...
          onClick={() => {
            ...
            setIsLocationFull(false); // ✅ Reset khi quay lại
          }}
        >
          Quay lại
        </Button>
  ...
  return (
    <div className="p-4 max-w-md mx-auto min-h-screen bg-slate-50 pb-24">
      ...
      <div className="flex gap-1 mb-4 px-1">
        ...
        <div
          className={`h-1.5 flex-1 rounded-full ${
            session.step === "INPUT_DETAILS" ? "bg-orange-500" : "bg-slate-200"
          }`}
        />
      </div>
      ...
      {/* Scanner Input (hidden on INPUT_DETAILS step) */}
      {session.step !== "INPUT_DETAILS" && (
